...
for index, fila in dfPuntosReferencia.iterrows(): 
    ...
    folium.Marker(
        ...
        tooltip=f"{str(fila['categoria']).capitalize()} {fila['nombre']}",
        icon=iconEstilo
    ...
    "features": [
        {
            ...
            "properties": {
                ...
                "search_index": f"{ str(punto['categoria']).capitalize()} - {punto['nombre']} - {' '.join(punto['detalles'])}"
            }
